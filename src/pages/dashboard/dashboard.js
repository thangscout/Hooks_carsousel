...
function Slide({ isCurrent, takeFocus, image, id, title, children }) {
	...
	return (
		<li ref={ref} aria-hidden={!isCurrent} tabIndex="-1" aria-labelledby={id} className="Slide"
...
function ProgressBar({ animate, time}) {
	let progress = Progress( animate, time);
...
function SpacerGif({ width}) {
	return (
		<div style={{ display: "inline-block", width}}></div>
	);
}
...
export default function Dashboard(){
	...
	useEffect(() => {
		...
		return () => window.removeEventListener("keydown", hanlder);
	})
		...
		<Carousel aria-label="Images from Space">
			<Slides>
				{slides.map((image, index) => (
					<Slide key={index} id={`image-${index}`} image={image.img} title={image.title}
					isCurrent={index === state.currentIndex} takeFocus={state.takeFocus} children={image.content}/>
				))}
			</Slides>
			<SlideNav>
			...
			<ProgressBar key={state.currentIndex + state.isPlaying} time={SLIDE_DURATION} animate={state.isPlaying}/>
			<VisuallyHidden>
				<Alert>
